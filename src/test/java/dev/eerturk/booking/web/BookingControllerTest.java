...
class BookingControllerTest {
  ...
  void whenDeleteBookingNotExistingThenShouldReturn404() throws Exception {
    ...
    doThrow(BookingNotFoundException.class).when(bookingService).delete(id);
    mockMvc.perform(delete("/bookings/" + id)).andExpect(status().isNotFound());
  }
  ...
  void whenRebookingDateNotAvailableThenShouldReturn422() throws Exception {
    ...
    doThrow(ReservationAlreadyExistsException.class).when(bookingService).rebook(id);
    mockMvc.perform(put("/bookings/" + id + "/rebook")).andExpect(status().is4xxClientError());
  }
  ...
  void whenCreateBookingSuccessfullyThenShouldReturn200() throws Exception {
    ...
    BookingDetailResponse expected =
