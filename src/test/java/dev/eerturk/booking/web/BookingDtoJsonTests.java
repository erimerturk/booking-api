...
public class BookingDtoJsonTests {
  ...
  @Autowired private JacksonTester<BookingDetailResponse> responseJson;

  @Autowired private JacksonTester<CreateBookingRequest> requestJson;

  @Test
  void testSerialize() throws Exception {
    var booking =
        new BookingDetailResponse(
            1l, 2222l, LocalDate.now(), LocalDate.now().plusDays(5l), 2111l, Status.ACTIVE);
    ...
    assertThat(jsonContent)
        .extractingJsonPathNumberValue("@.propertyId", Long.class)
        .satisfies(
            number -> assertThat(number.longValue()).isEqualTo(booking.propertyId().longValue()));

    assertThat(jsonContent)
        .extractingJsonPathNumberValue("@.guestId", Long.class)
        .satisfies(
            number -> assertThat(number.longValue()).isEqualTo(booking.guestId().longValue()));

    assertThat(jsonContent)
        .extractingJsonPathValue("@.status")
        .satisfies(data -> assertThat(Status.valueOf((String) data)).isEqualTo(booking.status()));

    assertThat(jsonContent)
        .extractingJsonPathValue("@.startDate")
        .satisfies(
            data -> assertThat(LocalDate.parse((String) data)).isEqualTo(booking.startDate()));

    assertThat(jsonContent)
        .extractingJsonPathValue("@.endDate")
